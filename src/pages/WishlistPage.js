...
function WishlistPage() {
	...
	return (
		<Container className={styles.container}>
			...
			{courses.length === 0 ? (
				<>
					<Warn className={styles.emptyList} title='담아 놓은 코스가 없어요.' description='카탈로그에서 나에게 필요한 코스를 찾아보세요.' />
				...
				<ul className={styles.items}>
					{courses.map((course) => (
						<li key={course.slug} className={styles.item}>
							...
							<img className={styles.delete} src={closeButton} alt='닫기' onClick={() => handleDelete(course.slug)} />
						</li>
					))}
				</ul>
			)}
